...
public class EvalTests
{
    ...
    {
        var stream = SetupTestsAi.ApiClient.StreamChatResponse(new Api.ChatRequestDto()
        {
            Messages = [
                new()
                {
    ...
    new ChatResponse(new ChatMessage(
        ...
        if (evaluationResults.Metrics.Values.Any(m => m.Interpretation?.Rating == EvaluationRating.Inconclusive))
        {
