...
public class AwsServerlessEndpointBuilder
{
    public static async Task Build()
    {
        ...
        foreach (var (type, attribute) in endpointTypes)
        {
            ...
            var (sanitizedRoute, routeParameters) = ParseRouteTemplate(route);

            var fileContents = BuildLambdaPartial(
                classNamespace,
                className,
                requestTypeName,
                requestType,
            ...
            var outputPath = Path.Combine(outputDir, $"{className}.Lambda.g.cs");
    ...
    static string BuildLambdaPartial(
    ...
    )
    {
        ...
        builder.AppendLine("using Amazon.Lambda.Annotations;");
        builder.AppendLine("using Amazon.Lambda.Annotations.APIGateway;");
        builder.AppendLine();
        builder.AppendLine($"namespace {classNamespace};");
        builder.AppendLine();
        builder.AppendLine($"public partial class {className}");
        builder.AppendLine("{");
        builder.AppendLine("    [Amazon.Lambda.Annotations.LambdaFunction()]");
        builder.AppendLine(
            $"    [Amazon.Lambda.Annotations.APIGateway.HttpApi(Amazon.Lambda.Annotations.APIGateway.LambdaHttpMethod.{http}, \"{route}\")]"
        );
        var signature = BuildMethodSignature(requestTypeName, requestType, routeParameters);
        builder.AppendLine(
            $"    public async System.Threading.Tasks.Task<Amazon.Lambda.Annotations.APIGateway.IHttpResult> {className}Lambda({signature}, Amazon.Lambda.Core.ILambdaContext _)"
        );
        builder.AppendLine("    {");
    ...
    )
    {
        ...
        {
            return $"[FromBody] {requestTypeName} request";
        }
