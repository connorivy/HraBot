...
public sealed class VectorStoreWriterDebug<T> : IngestionChunkWriter<T>
{
    ...
    public VectorStoreWriterDebug(VectorStore vectorStore, int dimensionCount, VectorStoreWriterOptions? options = default)
    {
        _vectorStore = vectorStore ?? throw new ArgumentNullException(nameof(vectorStore));
        _dimensionCount = dimensionCount <= 0 ? throw new ArgumentOutOfRangeException(nameof(dimensionCount)) : dimensionCount;
    ...
    {
        _ = chunks ?? throw new ArgumentNullException(nameof(chunks));
